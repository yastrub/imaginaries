...
router.get('/', async (req, res) => {
  try {
    ...
    const data = result.rows.map((r) => ({
      ...
      maxGenerationsPerMonth: r.max_generations_per_month ?? 0,
      maxFreeGenerations: r.max_free_generations ?? 0,
      maxGenerationsPerMonthEffective: (Number(r.max_generations_per_month || 0) + Number(r.max_free_generations || 0)) || 0,
      requiresWatermark: r.show_watermark,
      allowPrivateImages: r.allow_private_images,
      featuresText: r.features_text || null,

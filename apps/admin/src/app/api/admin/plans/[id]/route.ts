...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const res = await query(`
      SELECT id, key, name, description, max_generations_per_day, max_generations_per_month, max_free_generations, stripe_product_id, stripe_price_monthly_id, stripe_price_annual_id, show_watermark, allow_private_images,
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const allowed = ['key','name','description','max_generations_per_day','max_generations_per_month','max_free_generations','stripe_product_id','stripe_price_monthly_id','stripe_price_annual_id','show_watermark','allow_private_images','price_cents','annual_price_cents','currency','is_active','is_public','sort_order'] as const;
    const fields: string[] = [];
    const values: any[] = [];
    let idx = 1;

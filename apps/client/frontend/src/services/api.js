...
export async function generateImage(prompt, userId = null, drawingPng = null, drawingSvg = null, isPrivate = true) {
  try {
    if (!userId) {
    ...
    const requestKey = `${userId}-${hasDrawing ? 'with-drawing' : 'no-drawing'}-${Date.now()}`;
